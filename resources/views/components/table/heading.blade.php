...
>
    @unless ($sortable)
        <span class="text-xs font-medium leading-4 tracking-wider text-left text-gray-800 uppercase">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-xs font-medium leading-4 tracking-wider text-left text-gray-800 uppercase group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
